...
cxxflags = ("-g -c -Os -std=c++0x -fno-rtti -fno-exceptions " +
            "-ffunction-sections -fdata-sections " +
            "-fsingle-precision-constant " +
            "-DF_CPU=16000000L -mmcu=" + MCU + " " +
            get_defines() + " " + get_includes())

lflags = ("-g -Os -Wl,--gc-sections,-u,main " +
          "-fsingle-precision-constant -mmcu=" + MCU + " " + #" -T memory.x -T periph.x " +
          #"lib/mmpy-16/libgcc.a lib/mmpy-16/libgcov.a lib/mmpy-16/libcrt0.a " +
          #"lib/mmpy-16/libcrt0dwdt.a $lib_path/libm.a " +
          #"$lib_path/libc.a $lib_path/libfp.a ")
          "")
...
def write_buildfile():
    with open("build.ninja", "w") as buildfile:
        ...
        # Rule declarations
        n.rule("cxx",
               command = "$tc_path/msp430-g++ $cxxflags -c $in -o $out")

        n.rule("cc",
               command = "$tc_path/msp430-gcc $cflags -c $in -o $out")

        n.rule("cl",
               command = "$tc_path/msp430-gcc $lflags $in -o $out -lm -lgcc -lc")

        n.rule("oc",
               command = "$tc_path/msp430-objcopy -O binary $in $out")
